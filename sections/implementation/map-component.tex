...
seatgenApiClient.api.getSectorByMap(bucketName, mapName).then((r) => {
    if (r.ok) {
        context.setStandingAreas(r.data.map((data) => {
            const standingArea: StandingArea = {
                id: data.id!,
                name: data.name!,
                capacity: data.capacity,
                coordinates: data.coordinates?.map((c) => {
                    return new LatLng(c.x!, c.y!);
                })??[],
                selected: false
            }
            return standingArea
